...
class TestPromotePipeline(IsolatedAsyncioTestCase):
    ...
    async def test_run_skips_blocker_check_when_basis_assembly_has_included_bugs(
        self, load_group_config: AsyncMock, load_releases_config: AsyncMock, _, __
    ):
        """Blocker bug check skipped when assembly has basis.assembly AND issues.include (targeted fix)."""
        runtime = MagicMock(
            config={
                "build_config": {
                    "ocp_build_data_url": "https://example.com/ocp-build-data.git",
    ...
    async def test_check_blocker_bugs_no_excluded_bugs(self, cmd_gather_async: AsyncMock):
