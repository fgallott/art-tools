...
class TestBuildRpmsInYaml(unittest.TestCase):
    ...
    def test_arch_specific_upgrade_packages(self):
        ...
        result = build_rpms_in_yaml(
            ...
            arches=["x86_64", "ppc64le"],
            packages=[],
            upgrade_packages=[ArchSpecificPackage(name="dmidecode", arches={"only": "ppc64le"})],
        )

        self.assertEqual(
            result.upgradePackages,
            [ArchSpecificPackage(name="dmidecode", arches={"only": "ppc64le"})],
        )
        self.assertEqual(
            result.model_dump()["upgradePackages"],
            [{"name": "dmidecode", "arches": {"only": "ppc64le"}}],
        )
...
class TestBareUpdateUpgradeResolution(unittest.TestCase):
    ...
    def test_arch_scoped_upgrade_targets_are_not_deduplicated_by_name(self):
        """
        Architecture-scoped upgrade targets with the same name must retain each scope.
        """
        container = MagicMock(spec=ContainerImageHelper)
        container.resolve_to_digest = AsyncMock(side_effect=lambda p: p)
        container.get_installed_packages = AsyncMock(return_value=[])
        ...
        scoped_upgrade_targets = [
            ArchSpecificPackage(name="glibc", arches={"only": "x86_64"}),
            ArchSpecificPackage(name="glibc", arches={"only": "aarch64"}),
        ]
        repos = [RepoEntry(repoid="baseos", baseurl="https://example.com/$basearch/")]
        ...
        result = asyncio.run(
            generator._resolve_stage_with_retry(
                ...
                upgrade_packages=scoped_upgrade_targets,
            )
        )
