...
def _package_key(package: str | ArchSpecificPackage) -> tuple[str, tuple[tuple[str, str], ...]]:
    """
    Return a deduplication key that preserves architecture scope.

    Arg(s):
        package (str | ArchSpecificPackage): Package specification.
    Return Value(s):
        tuple[str, tuple[tuple[str, str], ...]]: Package name and normalized scope.
    """
...
class RpmLockfilePrototypeGenerator:
    ...
    async def _get_base_image_packages(
        ...
        and conflict detection. Queries every target architecture and falls
        back to parent lockfile data if the image is unreachable.
        ...
        Return Value(s):
            list[str | ArchSpecificPackage]: Package names or architecture-scoped package specifications.
        """
        if image_pullspec:
    ...
    async def _resolve_stage_with_retry(
        ...
        while real_retries < MAX_RESOLUTION_RETRIES:
            ...
            if remaining_upgrade:
                existing = {_package_key(package) for package in all_upgrade_targets}
                for package in remaining_upgrade:
                    package_key = _package_key(package)
                    if package_key not in existing:
                        all_upgrade_targets.append(package)
